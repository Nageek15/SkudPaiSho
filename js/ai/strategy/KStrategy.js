...
*/
function KStrategy(observer){
	Strategy.call(this,observer); // < - super constructor
	//stance 0 - defensive
	//stance 1 - neutral
	//stance 2 - offensive
	this.stance = 1;
...
KStrategy.prototype = Object.create(Strategy.prototype); // < - Link prototypes

KStrategy.prototype.calculateScore = function(origGame, copyGame) {
	// Simple move scoring...
	var score = 0;
...
KStrategy.prototype.ensurePlant = function(move, game, player,boardTiles,gateTiles) {
...
KStrategy.prototype.getBestMove = function(game, moveNum,moves){
	var randomIndex = Math.floor(Math.random() * moves.length);
...
KStrategy.prototype.calculatePlantScore = function(move,game,player,boardTiles,gateTiles){
	if (move.moveType!==PLANTING){
		return 0;
	}
